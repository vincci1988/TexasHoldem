...
public class WinRateEstimator_LSTM extends EstimatorBase {
	...
	WinRateEstimator_LSTM(AshePlus ashe) {
		super(ashe);
		lstm = new LSTMLayer(inputDim, cellCnt);
		fNet = new FFNetwork(cellCnt, hiddenNodeCnt, outputDim);
	}

	WinRateEstimator_LSTM(AshePlus ashe, double[] genome) throws Exception {
		super(ashe);
		lstm = new LSTMLayer(inputDim, cellCnt,
	...
	private double adjustHandStrength(double handStrength, ActionBase action, TableInfo info) {
		...
		if (action instanceof AllIn) {
			...
			if (ashe.getMyStack() + ashe.getMyBet() > info.currentBet) 		
				potOdds = 1.0 * (ashe.getMyStack() + ashe.getMyBet() - info.currentBet) / Params.stk / 2;
			else 
				potOdds = 1.0 * (info.currentBet - ashe.getMyBet()) / Params.stk / 2;
			if (handStrength < Math.sqrt(potOdds))
				return 0;
